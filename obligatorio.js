...
let solicitud4 = new Solicitudes(5, "Refrigerados", "Farmaceutica", "Lima", 12, "pendiente", 4, 2)
...
function entrarAlPrograma() {
    ...
    if (document.querySelector("#txtUsuarioLogin").value == "" || document.querySelector("#txtPassLogin").value == "" || document.querySelector("#slcUsuario").value == "Vacio") {
        ...
        if (coincideEmpresa == true) {
            ...
            document.querySelector("#msgAlertaInicioSesion").innerHTML = "Los datos ingresados son incorrectos. <br>Intente nuevamente";
        }
    }
}
...
function crearViajeEmpresa() {
    ...
    if (camposCompletos == true) {
        ...
        document.querySelector("#msgCrearViajeEmpresa").innerHTML = `Viaje creado con éxito. <br>El número de viaje es : ${unViaje.Identificador}.`
        generarComboDinamicoViajesListado()
        generarComboDinamicoViajes()
    }
    document.querySelector("#txtNombreBuqueEmpresa").value = ""
    ...
    for (let solicitud of listaSolicitudes) {
        if (solicitud.IdImportador == ImportadorDeshabilitado) {
            if (solicitud.Estado == "cancelado") {
                ...
                if (conocerImportador(ImportadorDeshabilitado).Estado == "deshabilitado") {
                    conocerImportador(ImportadorDeshabilitado).Estado = "habilitado"
        ...
        } else if(ImportadorDeshabilitado==""){
            document.querySelector("#msgImportadorHabilitado").innerHTML = "No hay importadores deshabilitados"
        }
    }
...
function conocerImportador(unId) {
    let encontrado = null
    for (let importador of listaImportadores) {
        if (importador.Identificador == unId) encontrado = importador
    }
    return encontrado
}
...
function dibujarTablaCargasdeViaje(viaje) {
    ...
    for (let solicitud of listaSolicitudes) {
        ...
        if (solicitud.IdViaje == viaje && solicitud.Estado == "confirmado") {
            viajeVacio = false
            tablaSolicitudes += `
            <tr>
...
function dibujarTablaCargasPeligrosas(viaje) {
    ...
    for (let solicitud of listaSolicitudes) {
        if (solicitud.IdViaje == viaje) {
            if (solicitud.TipoMercaderia == "Carga peligrosa" && solicitud.Estado == "confirmado") {
                viajeVacio = false
